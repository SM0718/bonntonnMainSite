...
function BestSellers() {
  ...
  const bestSellers = [
    ...
    },
    {
      ...
      picDesc: "Decadent hazelnut mousse with a velvety chocolate core."
    },
    {
      ...
      picDesc: "Artisanal truffles dusted with fine cocoa powder."
    },
    {
      ...
      picDesc: "Warm chocolate cake with a flowing molten center."
    },
  ];
  ...
  return (
    <div className="bg-white text-black py-8 sm:py-12 md:py-16 relative overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        <div className="flex flex-col gap-4 sm:gap-6 md:gap-8 justify-between bg-white text-black pb-8 sm:pb-12 md:pb-16">
          ...
          <p className="w-full md:w-3/4 lg:w-[800px] px-4 times text-[16px] sm:text-[17px] md:text-[18px] lg:text-[20px] text-center text-[#757575] mx-auto italic font-light leading-relaxed">
            Discover our best selling products crafted with premium ingredients 
          ...
          <Slider ref={sliderRef} {...settings}>
            {bestSellers.map((item, index) => (
              ...
              >
                ...
                <div className={`bg-white rounded-lg sm:rounded-xl md:rounded-2xl overflow-hidden transition-all duration-500 shadow-sm hover:shadow-md ${activeSlide % bestSellers.length === index ? 'scale-100' : 'scale-95 opacity-80'}`}>
                  ...
                  <div className="relative w-full h-[250px] sm:h-[280px] md:h-[320px] lg:h-[350px] xl:h-[380px] overflow-hidden group">
                    <img
                      ...
                      className={`h-full w-full object-cover transition-all duration-700 ${isHovering === index ? 'scale-105' : 'scale-100'}`}
                      src={item.pic}
                      alt={item.name}
                    />
                    
                    {/* Refined overlay effect with gradient */}
                    <div className={`absolute inset-0 bg-gradient-to-t from-black/50 via-black/10 to-transparent transition-opacity duration-500 ${isHovering === index ? 'opacity-100' : 'opacity-0'}`}></div>
                    
                    {/* Elegant price tag that appears on hover - adjusted sizes */}
                    <div className={`absolute top-3 sm:top-4 right-3 sm:right-4 bg-white text-black times py-1 sm:py-2 px-3 sm:px-4 md:px-5 rounded-full shadow-md transition-all duration-500 ${isHovering === index ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4'}`}>
                    ...
                    <div className={`absolute bottom-0 left-0 right-0 flex justify-center transition-all duration-500 ${isHovering === index ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
                      <button 
                        onClick={() => navigate(item.picSlug)}
                        className="bg-white/95 text-black times text-xs sm:text-sm tracking-wider uppercase py-2 sm:py-3 px-4 sm:px-6 md:px-8 mb-4 sm:mb-6 rounded-full shadow-md hover:bg-white transition-colors border border-gray-50"
                      >
                        Checkout Product
                      </button>
                    </div>
                  ...
                  <div className="px-4 sm:px-5 md:px-6 py-3 sm:py-4 md:py-5 transition-all duration-300">
                    <div className="flex flex-col space-y-2 sm:space-y-3">
                      <p className="times text-base sm:text-md tracking-wide font-medium">{item.name}</p>
