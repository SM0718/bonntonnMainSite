          ...
          {carouselItems.map((item, index) => (
            <div key={index}>
              <img 
                ...
                className="w-full object-fit h-52 rounded-lg"
      ...
      <ul className="grid grid-cols-3 gap-8 p-8 w-full bg-slate-100 z-40 times">
        {items.subItems?.map((menuItem, index) => (
          <li key={index} className="flex flex-col items-center">
            ...
            <span className="font-medium text-center">{menuItem.name}</span>
          </li>
        ))}
      </ul>
    );
      ...
      <ul className="flex flex-wrap px-12 py-4 gap-20 bg-slate-100 times">
        {items.subItems?.map((menuItem, index) => {
          ...
          return menuItem.name && (
            <li key={index} className="flex flex-col items-start justify-start">
              <div className="w-full h-full flex flex-col flex-wrap gap-6">
                <NavLink to={`/catagory/${menuItem.name}`} className="test-4xl font-medium">{menuItem.name}</NavLink>
                {menuItem.pic && (
  ...
  return (
    <>
      <div className="w-full hidden lg:flex flex-col z-50">
          ...
          <div className="w-full h-full flex justify-center items-center gap-8">
            {megaMenu.map((item, firstIndex) => (
              ...
              >
                <NavLink
                  to={item.slug}
                  className="trajan relative before:content-[''] before:absolute before:w-0 before:h-1 before:bg-[#BD9153] before:-bottom-4 before:left-1/2 before:-translate-x-1/2 before:transition-all before:duration-400 before:rounded-b-3xl group-hover:before:w-full group-hover:font-black font-bold text-sm text-[#757575] z-40"
                >
                  {item.name}
                </NavLink>
        ...
        <div className="hidden md:flex justify-evenly py-2 times bg-[#BD9153]">
